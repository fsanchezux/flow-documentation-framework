...
<div class="fd-code-block">
  ...
  class FlowDocsInstance {
    ...
    _apiBase() { return this.apiUrl.replace(/\/+$/, '') }
    _isAshx() { return /\.ashx(\?|$)/i.test(this._apiBase()) }
    _apiUrl(action) {
      const b = this._apiBase()
      return this._isAshx() ? `${b}?action=${action}` : `${b}/${action}`
    }

    async _fetchJson(action, options = {}) {
    ...
    async _initGitHubButtons() {
      let status
      try { status = await this._fetchJson('oauth-status') } catch (_) { return }
      if (!status.githubEnabled) return
      this.$.btnPush.classList.remove('fd-hidden')
      this.$.btnPull.classList.remove('fd-hidden')
      this.$.btnPush.addEventListener('click', (e) => { e.stopPropagation(); this._handlePush() })
      this.$.btnPull.addEventListener('click', (e) => { e.stopPropagation(); this._handlePull() })
    }
    ...
    async _ensureAuth() {
      ...
      if (!popup) throw new Error('Permite las ventanas emergentes para autenticarte con GitHub')
      await new Promise((resolve, reject) => {
        const handler = (e) => {
          if (!e.data || e.data.type !== 'flow-docs-oauth') return
          window.removeEventListener('message', handler)
          if (e.data.ok) resolve()
          else reject(new Error(e.data.error || 'Auth cancelada'))
        }
        window.addEventListener('message', handler)
        const poll = setInterval(() => {
          if (popup.closed) { clearInterval(poll); window.removeEventListener('message', handler); reject(new Error('Ventana cerrada')) }
        }, 500)
      })
    ...
    async _handlePush() {
      try {
        ...
        if (!status.canWrite) throw new Error(`${status.user} no tiene permiso de escritura en el repo`)
        if (!status.pending) { this._showToast('Nada que enviar'); return }
        ...
        if (result.upToDate) { this._showToast('Ya está al día'); return }
    ...
    async _handlePull(force = false) {
      try {
        ...
        // Reload data
        if (this.apiUrl) this._loadFromApi()
      } catch (e) {
        this._showToast('Error: ' + e.message)
      } finally {
