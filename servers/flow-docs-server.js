 ...
 *
 * v2.0: Local save only (writes to disk + JSONL buffer).
 *
 * GitHub OAuth + Push + Pull are only implemented in the ASP.NET handler
 * (FlowDocsHandler.ashx) in this release. Node OAuth support is planned
 * for 2.1 — track issue on the repo.
 *
 * Usage:
 ...
 * Endpoints:
 *   GET  /         → full JSON data
 *   POST /save     → write file + append JSONL buffer
 */
...
function buildData(docsDir) {
  ...
  if (!fs.existsSync(resolved)) return { version: 1, skills: [], flags: SEARCH_FLAGS, github: { enabled: false } }
  ...
  for (const entry of entries) {
    ...
    if (homeKey && homeKey !== 'home.md') { files['home.md'] = files[homeKey]; delete files[homeKey] }
    const descSource = files['home.md'] || files['SKILL.md']
    skills.push({ name: entry.name, description: descSource ? extractDescription(descSource) : '', files })
  }
  skills.sort((a, b) => a.name.localeCompare(b.name))
  const homeMd = path.join(resolved, 'HOME.md')
  const homePage = fs.existsSync(homeMd) ? fs.readFileSync(homeMd, 'utf-8') : null
  const result = { version: 1, generatedAt: new Date().toISOString(), skills, flags: SEARCH_FLAGS, github: { enabled: false } }
...
module.exports = function flowDocsServer(docsDir) {
  ...
  router.get('/', (req, res) => res.json(buildData(docsDir)))
  router.post('/save', express.json({ limit: '10mb' }), (req, res) => {
    try {
      const { skill, file, content, user } = req.body
      if (!skill || !file || typeof content !== 'string') return res.status(400).json({ error: 'Missing skill, file, or content' })
      const resolved = path.resolve(docsDir)
      ...
      appendChangeLog(docsDir, {
        ...
        skill, file, bytes: Buffer.byteLength(content, 'utf-8'), created, content
      })
      res.json({ success: true })
    } catch (e) { res.status(400).json({ error: e.message }) }
  })
  // GitHub OAuth + Push/Pull endpoints: not implemented in Node yet (use ASHX).
  router.get('/oauth-status', (req, res) => res.json({ githubEnabled: false, authenticated: false, pending: 0 }))
  return router
}
